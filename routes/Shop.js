            ...
            async function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    if (body.Error !== undefined) {return res.render('errorCustom', { error: body.Error });} else {
                        return res.render('./Shop/index', { title: req.WebTitle+'Shop', Option: "Index", Data: body, Login: await await req.isLogin()(), Admin: await req.isStaff() });
                    }
            ...
            async function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    if (body.Error !== undefined) {
                        ...
                        return res.render('./Shop/index', { title: req.WebTitle+'Shop - '+req.params.Category, Option: "Category", Category: req.params.Category, Data: body, Login: await await req.isLogin()(), Admin: await req.isStaff() });
                    }
                } else return res.render('errorCustom', { error: "API: Response Error" });
            }
        );
