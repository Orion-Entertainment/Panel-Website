...
router.get('/', async(req, res, next) => {
    try {
        return res.render('index', { title: req.WebTitle+'Home', Login: await await req.isLogin()() });
    } catch (error) {
        ...
        request.post(
            ...
            async function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    else return res.render('./Changelog/Index', { title: req.WebTitle+'Changelog', Login: await await req.isLogin()(), Admin:await req.isStaff(), Data: JSON.stringify(body) });
                } else return res.render('errorCustom', { error: "API: Response Error" });
            }
        );
    } catch (error) {
    ...
    try {
        ...
        request.post(
            ...
            async function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    if (body.Error !== undefined) return res.render('errorCustom', { error: body.Error });
                    else return res.render('./Changelog/View', { title: req.WebTitle+'Changelog', Login: await await req.isLogin()(), Admin:await req.isStaff(), Data:body });
                } else return res.render('errorCustom', { error: "API: Response Error" });
            }
        );
    } catch (error) {
